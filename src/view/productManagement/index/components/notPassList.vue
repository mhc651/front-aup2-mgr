...
<template>
  <div class="wrap">
    <div class="filterWrap">
      ...
      <div class="gird20">
        <el-date-picker style="width: 100%" v-model="param.timeArr" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd HH:mm:ss"> </el-date-picker>
      </div>
      <div class="gird20">
    ...
    <el-table :data="listData" style="width: 100%; margin-top: 20px" @selection-change="handleSelectionChange" v-loading="mainLoading" @row-click="toDetail">
      ...
      <el-table-column prop="auditOpinion" label="审核意见" width="120px">
        <template slot-scope="scope">
          <el-tooltip placement="top-start">
            <div slot="content">{{scope.row.auditOpinion}}</div>
            <p style="width: 100px; overflow: hidden;text-overflow:ellipsis;white-space:nowrap;">{{scope.row.auditOpinion}}</p>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    data() {
      return {
        param: {
          ...
          auditStatus: 5,  //审核状态,//2:待审核,3:审核中,4:通过,5:未通过
          auditName: '' , // 审核人
          projectType: '', // 竞价分组
          auditStart: '', // 审核时间起,
          auditEnd: '', // 审核时间止,
    ...
    methods: {
      ...
      async getList() {
        ...
        reqData.auditStart = reqData.timeArr && reqData.timeArr.length === 2 ? reqData.timeArr[0] : ''
        reqData.auditEnd = reqData.timeArr && reqData.timeArr.length === 2 ? reqData.timeArr[1] : ''
