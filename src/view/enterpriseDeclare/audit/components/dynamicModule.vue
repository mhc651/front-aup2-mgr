...
<template>
  <div class="wrap">
    ...
    <div class="bd">
      ...
      <div class="dsf bdb" :class="{bdRed:competeValue(item.code)}" v-for="(item,index) in dataField.attributes" :key="index">
        ...
        <div class="td70 bdr fdc ais jcc">
          <!--文件类型-->
        ...
        <div class="td15 jcc"><tickGroup :audit="audit" :field="item.code" /></div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      competeValue (key) {
        return this.reaudit && this.valueField[key] !== this.history[key]
      }
    },
    created() {
    },
