...
<template>
  <div class="wrap">
    <div class="filterWrap">
      ...
      <div class="gird20">
        <el-select v-model="param.acceptStatus" placeholder="请选择分配状态" style="width: 100%;">
    ...
    <el-table
      ...
      <el-table-column label="分配状态">
        <template slot-scope="scope">{{ scope.row.acceptStatus === '1' ? '未分配':scope.row.acceptStatus }}</template>
      </el-table-column>
      <el-table-column label="企业类型">
        <template slot-scope="scope">{{ scope.row.projectType == 1 ? '生产企业':'代理企业' }}</template>
      </el-table-column>
  ...
  export default {
    data() {
      return {
        param: {
          ...
          acceptStatus: null,       // 分配状态,//1:未分配,2:已分配
          auditStatus: 3,           // 审核状态, 1:待提交,2:待审核,3:审核中,4:审核通过,5:审核未通过,6:已取消
    ...
    methods: {
      ...
      audit(e) {
        ...
        this.$router.push('/enterpriseDeclareAudit?id=' + this.declareSelection[0].id)
      },
