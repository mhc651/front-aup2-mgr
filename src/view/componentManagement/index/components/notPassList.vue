...
<template>
  <div class="wrap">
    ...
    <el-table :data="listData" style="width: 100%; margin-top: 20px" v-loading="mainLoading" @selection-change="handleSelectionChange" @row-click="toDetail">
      ...
      <el-table-column prop="auditOpinion" label="审核意见" width="250px">
        <template slot-scope="scope">
          <el-tooltip placement="top-start" >
            <div slot="content">{{scope.row.auditOpinion}}</div>
            <p style="width: 230px; overflow: hidden;text-overflow:ellipsis;white-space:nowrap;">{{scope.row.auditOpinion}}</p>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    <withdraw :id="currentId" v-model="showWithdrawDialog" url="aup2mgr/api/project/revokeSubGroupInfo" @success="revokeSuccess"/>
